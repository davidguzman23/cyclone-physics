...
RigidBodyApplication2::RigidBodyApplication2()
:
    theta(0.0f),
    phi(15.0f),
...
void RigidBodyApplication2::update()
{
    // Find the duration of the last frame in seconds
    float duration = (float)TimingData::get().lastFrameDuration * 0.001f;
    if (duration <= 0.0f) return;
...
void RigidBodyApplication2::display()
{
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
    glLoadIdentity();
...
void RigidBodyApplication2::drawDebug()
{
    ...
    for (unsigned i = 0; i < cData.contactCount; i++)
    {
        ...
        cyclone2::Vector3 vec = contacts[i].contactPoint;
...
void RigidBodyApplication2::mouse(int mouse, int state, int x, int y)
{
    // Set the position
    last_x = x;
    last_y = y;
...
void RigidBodyApplication2::mouseDrag(int x, int y)
{
    // Update the camera
...
void RigidBodyApplication2::key(unsigned char key)
{
    switch(key)
    {
    case 'R': case 'r':
