...
class AmmoRound : public cyclone2::CollisionSphere
{
public:
    ShotType type;
    unsigned startTime;
    ...
    void setState(ShotType shotType)
    {
        ...
        cyclone2::real coeff = 0.4f*body->getMass()*radius*radius;
 ...
 */
class BigBallisticDemo : public RigidBodyApplication2
{
    /**
...
void BigBallisticDemo::updateObjects(cyclone2::real duration)
{
...
void BigBallisticDemo::generateContacts()
{
    ...
    cData.restitution = (cyclone2::real)0.1;
    ...
    for (Box *box = boxData; box < boxData+boxes; box++)
    {
        ...
        for (AmmoRound *shot = ammo; shot < ammo+ammoRounds; shot++)
        {
            if (shot->type != UNUSED)
            {
                ...
                if (cyclone2::CollisionDetector::boxAndSphere(*box, *shot, &cData))
                {
                    shot->type = UNUSED;
                }
            }
