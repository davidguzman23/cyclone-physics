...
cyclone2::Random global_random;

class Block : public cyclone2::CollisionBox
{
public:
    bool exists;
    ...
    void setState(const cyclone2::Vector3 &position,
                  const cyclone2::Quaternion &orientation,
                  const cyclone2::Vector3 &extents,
                  const cyclone2::Vector3 &velocity)
    {
        body->setPosition(position);
        body->setOrientation(orientation);
        body->setVelocity(velocity);
        body->setRotation(cyclone2::Vector3(0,0,0));
        halfSize = extents;
     ...
     */
    void calculateMassProperties(cyclone2::real invDensity)
    {
        ...
        if (invDensity <= 0)
        {
            ...
            body->setInverseInertiaTensor(cyclone2::Matrix3());
        }
        else
        {
            // Otherwise we need to calculate the mass
            cyclone2::real volume = halfSize.magnitude() * 2.0;
            cyclone2::real mass = volume / invDensity;
     ...
     */
    void divideBlock(const cyclone2::Contact& contact,
        Block* target, Block* blocks)
    {
        ...
        cyclone2::Vector3 normal = contact.contactNormal;
        cyclone2::RigidBody *body = contact.body[0];
        if (body != target->body)
        {
            normal.invert();
            body = contact.body[1];
        ...
        // and its direction.
        cyclone2::Vector3 point = body->getPointInLocalSpace(contact.contactPoint);
        ...
        cyclone2::Vector3 size = target->halfSize;

        // Take a copy also of the body's other data.
        cyclone2::RigidBody tempBody;
        ...
        for (unsigned i = 0; i < 8; i++)
        {
            ...
            // local coordinates.
            cyclone2::Vector3 halfSize = (max - min) * 0.5f;
            cyclone2::Vector3 newPos = halfSize + min;

            // Convert the origin to world coordinates.
            newPos = tempBody.getPointInWorldSpace(newPos);

            // Work out the direction to the impact.
            cyclone2::Vector3 direction = newPos - contact.contactPoint;
            direction.normalize();
            ...
            blocks[i].body->setAcceleration(cyclone2::Vector3::GRAVITY);
            blocks[i].body->clearAccumulators();
            blocks[i].body->calculateDerivedData();
            blocks[i].offset = cyclone2::Matrix4();
 ...
 */
class FractureDemo : public RigidBodyApplication2
{
...
{
    // Create the ball.
    ball.body = new cyclone2::RigidBody();
    ball.radius = 0.25f;
    ...
    ball.body->setAcceleration(cyclone2::Vector3::GRAVITY);
...
void FractureDemo::generateContacts()
{
    ...
    cData.restitution = (cyclone2::real)0.2;
    ...
    for (Block *block = blocks; block < blocks+MAX_BLOCKS; block++)
    {
        ...
        cyclone2::CollisionDetector::boxAndHalfSpace(*block, plane, &cData);

        if (ball_active)
        {
            // And with the sphere
            if (!cData.hasMoreContacts()) return;
            if (cyclone2::CollisionDetector::boxAndSphere(*block, ball, &cData))
            {
                hit = true;
        ...
        for (Block *other = block+1; other < blocks+MAX_BLOCKS; other++)
        {
            ...
            if (!cData.hasMoreContacts()) return;
            cyclone2::CollisionDetector::boxAndBox(*block, *other, &cData);
        }
    }
    ...
    {
        if (!cData.hasMoreContacts()) return;
        cyclone2::CollisionDetector::sphereAndHalfSpace(ball, plane, &cData);
    }
}
...
void FractureDemo::reset()
{
    ...
    // Set the first block
    blocks[0].halfSize = cyclone2::Vector3(4,4,4);
    ...
    blocks[0].body->setAcceleration(cyclone2::Vector3::GRAVITY);
...
void FractureDemo::updateObjects(cyclone2::real duration)
{
    for (Block *block = blocks; block < blocks+MAX_BLOCKS; block++)
    {
        if (block->exists)
...
void FractureDemo::display()
{
    ...
    if (ball_active)
    {
        ...
        glPushMatrix();
        cyclone2::Vector3 pos = ball.body->getPosition();
