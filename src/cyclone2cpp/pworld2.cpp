...
{
    for (Particles2::iterator p = particles.begin();
        p != particles.end();
        p++)
    {
...
ParticleWorld::Particles2& ParticleWorld::getParticles()
{
    return particles;
}
...
void GroundContacts::init(cyclone2::ParticleWorld::Particles2 *particles)
{
    GroundContacts::particles = particles;
}

unsigned GroundContacts::addContact(cyclone2::ParticleContact *contact, unsigned limit) const
{
    unsigned count = 0;
    for (cyclone2::ParticleWorld::Particles2::iterator p = particles->begin();
        ...
        p++)
    {
        cyclone2::real y = (*p)->getPosition().y;
        if( y < 0.0f)
        {
            contact->contactNormal = cyclone2::Vector3::UP;
