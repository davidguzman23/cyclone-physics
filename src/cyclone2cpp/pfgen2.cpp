...
void ParticleForceRegistry::add(Particle2* particle, ParticleForceGenerator *fg)
{
...
void ParticleGravity::updateForce(Particle2* particle, real duration)
{
...
void ParticleDrag::updateForce(Particle2* particle, real duration)
{
    Vector3 force;
...
ParticleSpring::ParticleSpring(Particle2 *other, real sc, real rl)
: other(other), springConstant(sc), restLength(rl)
{
}

void ParticleSpring::updateForce(Particle2* particle, real duration)
{
...
void ParticleBuoyancy::updateForce(Particle2* particle, real duration)
{
...
ParticleBungee::ParticleBungee(Particle2 *other, real sc, real rl)
: other(other), springConstant(sc), restLength(rl)
{
}

void ParticleBungee::updateForce(Particle2* particle, real duration)
{
    // Calculate the vector of the spring
    Vector3 force;
    particle->getPosition(&force);
...
void ParticleFakeSpring::updateForce(Particle2* particle, real duration)
{
    // Check that we do not have infinite mass.
    if (!particle->hasFiniteMass()) return;
...
void ParticleAnchoredBungee::updateForce(Particle2* particle, real duration)
{
    // Calculate the vector of the spring
    Vector3 force;
    particle->getPosition(&force);
...
void ParticleAnchoredSpring::updateForce(Particle2* particle, real duration)
{
    // Calculate the vector of the spring.
    Vector3 force;
    particle->getPosition(&force);
